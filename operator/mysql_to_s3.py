...
class MysqlToS3Operator(BaseOperator):
    ...
    def __init__(self,
                 ...
                 aws_conn_id='google_cloud_default',
                 schema=None,
                 delegate_to=None,
                 export_format='json',
                 field_delimiter=',',
    ...
    def _upload_to_s3(self, files_to_upload):
        ...
        for tmp_file in files_to_upload:
            print(tmp_file)
            print(tmp_file.get('file_handle').name)
            hook.load_file(tmp_file.get('file_handle').name, tmp_file.get('file_name'), replace=True, bucket_name=self.bucket)
#            hook.load_file(self.bucket, tmp_file.get('file_name'),
